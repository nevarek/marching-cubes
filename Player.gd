...
func _handle_key_input(delta):
	if game_paused() == false:
		...
		if movement_direction.length() > 0:
			# do acceleration
			velocity = velocity.lerp(move_speed_this_frame * movement_direction, acceleration * delta)
		else:
			# do deceleration
			velocity = velocity.lerp(movement_speed * movement_direction, deceleration * delta)
